...
The main model has been updated to run using miepython for the Mie
theory computation. Miepython is an validated python package written by
Scott Prahl. Both miepython and the equivalent functions written by the
authors of PPOPS (contained in validation/mie_python.py) produce nearly
identical results. mie_modules.py is retained for illustrative and
testing purposes but is not used in the model.
...
- Higher-order terms computed via recurrence:
  $\pi_n = \frac{(2n-1)u\,\pi_{n-1} - n\,\pi_{n-2}}{n-1}, \quad
